...
namespace Web.File
{
    ...
    public class FileHandler : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                ...
                case "GetAll":
                    ...
                    break;
                case "GetFile":
                     filename = context.Request.Form["Name"].ToString();
                    context.Response.Write(GetFile(filename));
                    break;
                case "GetFile2":
                    filename = context.Request.Form["Name"].ToString();
                    context.Response.Write(GetFile2(filename));
                    break;
        ...
        {
            List<DocInfo> files = new List<DocInfo>();
            string sql = "SELECT * FROM `web`.`文档库` where 文档名 like '%" + filename + "%' and 机组编号='" + jzidstr + "'"; 
            Dictionary<string, object> d = new Dictionary<string, object>();
            //获取数据总数(注意是总数,不是一页中数据的条数)
            DataTable dTable = data.GetTable(sql);
            d.Add("total", data.GetTable(sql).Rows.Count);
