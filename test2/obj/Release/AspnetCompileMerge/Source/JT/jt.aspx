...
<script type="text/javascript">
    ...
    var x = 112.338058;
    var y = 30.304716;
    var ggPoint = new BMap.Point(x, y);//表示一个地理坐标点。
    //添加gps marker和label
    var markergg = new BMap.Marker(ggPoint);//定义标注点
    map.addOverlay(markergg); //添加添加标注点
    var labelgg = new BMap.Label('巨鲸传动', { offset: new BMap.Size(20, -10) });//定义文本标注
    markergg.setLabel(labelgg); //添加 label
   //markergg.setTitle( '我是刚进的');
    labelgg.hide();
    markergg.addEventListener('mouseover', mouseover);
    markergg.addEventListener('mouseout', mouseout);
    //var numLabel = new BMap.Label(labelgg);
    labelgg.setStyle({
        color: "#000",
        fontSize: "14px",
        backgroundColor: '#fff',
    ...
    //markergg.addEventListener('click', loop);
    var menuId;
    var Url;
    function loop(e) {
        var label = e.target.getLabel().content;
        myajax(label);
        self.location.href = Url+"?node="+menuId;

    }
    function myajax(menuName) {
        $.ajax({
            url: "/JT/JTHandler.ashx?action=GetMenuId",
            type: "post",
            async: false,
